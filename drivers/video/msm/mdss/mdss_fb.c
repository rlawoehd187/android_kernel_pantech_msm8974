...
void mdss_fb_set_backlight(struct msm_fb_data_type *mfd, u32 bkl_lvl)
{
	...
	u32 temp = bkl_lvl;
	printk("[LCD_ON_SPEED] %s : Framework backlight level Enter %d\n", __func__, bkl_lvl);
	if (((!mfd->panel_power_on && mfd->dcm_state != DCM_ENTER)
		|| !mfd->bl_updated) && !IS_CALIB_MODE_BL(mfd)) {
				printk("[LCD_ON_SPEED] %s : Framework backlight level Return %d\n",__func__, bkl_lvl);
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		printk("[LCD_ON_SPEED] %s : Framework backlight level Set %d\n", __func__ , bkl_lvl);
...
void mdss_fb_update_backlight(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->unset_bl_level && !mfd->bl_updated) {
		...
		if ((pdata) && (pdata->set_backlight)) {
			...
			printk("[LCD_ON_SPEED] %s : FB commit backlight Set %d\n",__func__, mfd->bl_level);
			     ...
			     int op_enable)
{
	...
	if (mfd->dcm_state == DCM_ENTER)
       ...
       printk("[LCD_ON_SPEED]%s+ : blank_mode =%d\n",__func__, blank_mode);
